...
def _list():
    ...
    return render_template('repayment/repayment_list.html', repayment_list=repayment_list)
...
def create():
    ...
    if request.form['remark']:
        ...
        remark = None
    now = datetime.now()
    repayment = Repayment(category=category, amount=amount, created_at=now, remark=remark)
    ...
    balance = Balance(repayment_id=repayment.id, balance=new_balance, created_at=now, remark=remark)
